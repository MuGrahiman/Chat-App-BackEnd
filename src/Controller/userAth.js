...
export const userLogin = async (req, res) => {
  ...
  const verifiedPassword =await Auth.Decrypt(req.body.password, existUser.password);
  console.log(verifiedPassword);
  if (!verifiedPassword)
    return res.status(404).json({ message: "password couldnt match" });
...
export const sendOtp = async (req, res) => {
  try {
    ...
    else return res.status(404).json({ message: "somthing went wrong" });
  } catch (error) {}
};
...
export const checkOtp = async (req, res) => {
  try {
    ...
    if (!otp) return res.status(404).json({ message: "otp not varified" });
    const existUser = await userModel.findById(otp.userId);
    if (!existUser)
      return res.status(404).json({ message: "user not varified" });
